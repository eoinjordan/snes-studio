...
const EXAMPLES = [
  { slug: 'pocket-bugs', name: 'Pocket Bugs', desc: 'Garden bug battler showcase' },
  ...
  return <section className="card preview-card">
    ...
    {romUrl
      ...
      : <div className="preview-empty">
          ...
          {onPlayBuiltIn ? <Btn className="primary" onClick={onPlayBuiltIn}><Play size={16}/>Play Pocket Bugs (showcase)</Btn> : null}
          ...
          <p className="hint">The built-in Pocket Bugs showcase ROM is generated by SNES Studio. Files stay in your browser.</p>
        </div>}
  </section>;
}
...
function App(){
  ...
  function playBuiltIn(){ setRom(prev=>{ if(prev?.url?.startsWith('blob:')) URL.revokeObjectURL(prev.url); const cacheBust = Date.now(); return { url: `${import.meta.env.BASE_URL}roms/pocket-bugs.sfc?cacheBust=${cacheBust}`, name: 'Pocket Bugs (showcase ROM)' }; }); setView('preview'); setLog('Loaded the Pocket Bugs showcase ROM.'); }
